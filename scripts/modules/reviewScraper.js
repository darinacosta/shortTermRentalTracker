var unirest = require('unirest'),
    ...
    today = new Date(),
    todayCalc = new Date(),
    past2weeks =  new Date(todayCalc.setDate(todayCalc.getDate() - 0)), //CHANGED FOR IMMEDIATE SCRAPING
    MongoClient = require('mongodb').MongoClient,
    assert = require('assert'),
    rentaldb = 'mongodb://localhost:27017/reviewtracker',
...
reviewScraper = {
  ...
  _generateRandomNumber: function(){
    var reviewScraper = this;
    var randomNumThreshold = Math.floor((Math.random() * 10) + 1);
    if (reviewScraper._apiPageTracker['airbnb']['start'] % 6 === 0){ 
  ...
  init: function(providers, startPage, stopPage) { 
    ...
    providers.forEach(function(provider){
      reviewScraper._apiPageTracker[provider] = {start: startPage, stop: stopPage};
      reviewScraper._fetchListingsByProvider(provider);
    });
  },
  ...
  _fetchListingsByProvider: function(provider){
    ...
    var reviewScraper = this,
        ...
        url = "https://zilyo.p.mashape.com/search?isinstantbook=false" + 
              ...
              "&&provider=" + provider + "&" +
              "page=" + reviewScraper._apiPageTracker[provider]['start'];
    ...
    function _getListings(result){
      var parsedResult;
      console.log('Scanning ' + provider + ' page ' + reviewScraper._apiPageTracker[provider]['start'] + ' of ' + reviewScraper._apiPageTracker[provider]['stop']);
      try {
      ...
      reviewScraper._apiPageTracker[provider]['start'] = reviewScraper._apiPageTracker[provider]['start'] + 1;
      reviewScraper._pageCount = reviewScraper._pageCount + 1;
      reviewScraper._handleApiPageResult(parsedResult, function(){
     	if ((parsedResult === null || parsedResult['ids'].length > 0) && reviewScraper._apiPageTracker[provider]['start'] < reviewScraper._apiPageTracker[provider]['stop']){
          reviewScraper._fetchListingsByProvider(provider)
        } else {
          reviewScraper._apiPageTracker[provider]['start'] = "complete";
  ...
  _detectScanCompletion: function(){
    var pageTracker = reviewScraper._apiPageTracker;
        resultList = [],
        scanComplete = true;
  ...
  _handleApiPageResult: function(result, cb){
    ...
    for (var i = 0; i < result['result'].length; i += 1){
      var feature = reviewScraper._buildFeature(result['result'][i]);
      features.push(feature);
    };

    mapSeries(features, reviewScraper._writeFeatureToDb)
    .then(
    function () { 
      cb();
    },
    ...
    MongoClient.connect(rentaldb, function(e, db) {
	  ...
	  function _addNewFeature(feature){
            db.collection('features').insert(feature, function(e, records) {
              ...
              reviewScraper._numberOfFeaturesWritten += 1;
              db.close();
              deferred.resolve();
            });
          };
          ...
          if (docs.length === 0){
            reviewScraper._scrapeListing(feature, function(listingFeature){
  ...
  _scrapeListing: function(feature, callback){
    ...
    function _getListingData(error, response, html){
      ...
      }, 1000 * reviewScraper._generateRandomNumber());
  ...
  _writeToLog: function(){
    ...
    "New features collected: " + reviewScraper._numberOfFeaturesWritten + "\n" +
    "--------------------------"+ "\n";
    console.log(logString);
    fs.appendFile(reviewScraper._logFile, logString);
  }
};
